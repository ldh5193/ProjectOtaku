...
export function buildDirectionsAppUrl(store: Store): string {
  const params = new URLSearchParams({
...
export function buildDirectionsWebUrl(store: Store): string {
  const dname = encodeURIComponent(store.name);
  return `https://map.naver.com/v5/directions/-/${store.lng},${store.lat},${dname}/-/transit`;
}
